...
#
# Then: upload <output-dir>/oncoref-data-v<DATA_VERSION>.tar.gz to the
# `v<DATA_VERSION>` release on pirl-unc/oncoref, and only THEN bump DATA_VERSION
...
echo "wrote $OUT ($SIZE)"
echo
echo "next: gh release upload v${DATA_VERSION} '$OUT' --repo pirl-unc/oncoref"
