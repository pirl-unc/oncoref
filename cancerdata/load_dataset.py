...
Two data roots are checked in order:
  ...
  1. ``_BUNDLED_DATA_DIR`` — files shipped in the wheel (the small curated
     tables) AND files present in a git checkout's ``cancerdata/data/``.
  2. the cache populated by :mod:`cancerdata.data_bundle` (the large per-cohort
...
:data:`cancerdata.data_bundle.DOWNLOADABLE_PATHS` items and it's missing from
both, :func:`cancerdata.data_bundle.ensure_local` triggers a one-time download.
"""
...
def _load_shard_directory(shard_dir: Path) -> pd.DataFrame:
    ...
    ``~/.cache/cancerdata/shard_cache/``, keyed on a signature of the shard
    ...
    cache_dir = Path.home() / ".cache" / "cancerdata" / "shard_cache"
