...
to the read-time accessors in :mod:`oncodata.expression`. Each shipped shard
...
Each shard's :class:`~oncodata.expression.ShardDataset` names its core via
``build_attr``, so :mod:`oncodata.expression` can regenerate a missing cohort
...
def sum_proteoform_tpm(
    ...
    *,
    ...
    (see :func:`oncodata.proteoforms.proteoform_symbol`). First-appearance row order
    is preserved.
...
def cohort_percentile_vectors(
    ...
    *,
    ...
    encoding :func:`oncodata.expression.cohort_gene_percentiles` restores with
